	...
	const fetchTransactions = useCallback(
		async (forceRefresh = false) => {
			...
			if (!isAuthenticated || !member) {
				Logger.debug(
					"Data",
					"Not authenticated or no member, skipping transaction fetch"
				);
				return;
			}
			...
			const isCacheValid =
				lastFetched && now - lastFetched < CACHE_EXPIRATION;

			if (isCacheValid && !forceRefresh && dataLength > 0) {
				Logger.debug("Data", "Using cached transactions data", {
					count: dataLength,
				});
				return;
			}
			...
			try {
				Logger.info("Data", "Fetching transactions", {
					memberId: member.id,
				});
		...
		async (forceRefresh = false) => {
			...
			if (!isAuthenticated || !member) {
				Logger.debug(
					...
					"Not authenticated or no member, skipping notifications fetch"
				);
				return;
			}

			// Clear notification cache when forcing refresh to ensure we get fresh data
			if (forceRefresh) {
			...
			if (isCacheValid && !forceRefresh) {
				Logger.debug(
					LogCategory.NOTIFICATIONS,
					"Using cached notifications data"
				);
				return;
			}
			...
			try {
				...
				Logger.debug(
					LogCategory.NOTIFICATIONS,
					`Fetched ${allData.length} notifications`
				);

				// Check if we found any jatuh_tempo notifications
				const jatuhTempoFound = allData.some(
					(notification) =>
				...
				if (jatuhTempoFound) {
					Logger.debug(
						LogCategory.NOTIFICATIONS,
						`Found jatuh_tempo notifications`
					);
				} else {
					Logger.debug(
						LogCategory.NOTIFICATIONS,
				...
				Logger.debug(
					...
					{
						types: typeCount,
						hasDueDateNotifications: jatuhTempoFound,
					}
				);
	...
	const markNotificationAsRead = useCallback(
		async (id: string): Promise<boolean> => {
			if (!isAuthenticated || !member) {
				Logger.debug(
					LogCategory.NOTIFICATIONS,
					"Not authenticated or no member, cannot mark notification as read"
				);
				return false;
			}
			...
			try {
				...
				setState((prev) => {
					...
					Logger.debug(
						LogCategory.NOTIFICATIONS,
						`Updated local state for notification ${id}`,
						{ unreadCount }
					);
	...
	const markAllNotificationsAsRead =
		useCallback(async (): Promise<boolean> => {
			if (!isAuthenticated || !member) {
				Logger.debug(
					LogCategory.NOTIFICATIONS,
					"Not authenticated or no member, cannot mark all notifications as read"
				);
				return false;
			}
			...
			try {
				...
				setState((prev) => {
					...
					Logger.debug(
						LogCategory.NOTIFICATIONS,
						"Updated local state for all notifications"
					);
