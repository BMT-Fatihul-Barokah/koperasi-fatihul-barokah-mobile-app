...
export default function PinjamanDetailScreen() {
	...
	const fetchLoanData = async () => {
		...
		try {
			...
			const loanData = await PembiayaanService.getPembiayaanById(
				loanId
			);
	...
	const getLoanIcon = (jenisPinjaman: string | undefined | null) => {
		if (!jenisPinjaman) return "money-bill-wave";

		const lowerCase = jenisPinjaman.toLowerCase();
		if (lowerCase.includes("usaha")) return "store";
		if (lowerCase.includes("pendidikan")) return "graduation-cap";
		if (lowerCase.includes("kesehatan")) return "heartbeat";
		if (lowerCase.includes("renovasi")) return "home";
		if (lowerCase.includes("kendaraan")) return "car";
				...
				<View style={styles.content}>
					<View style={styles.loanHeaderDetailed}>
						<View style={styles.loanHeaderContent}>
							<View
								style={styles.loanTypeContainer}
							>
								<FontAwesome5
									name={getLoanIcon(
					...
					<View style={styles.paymentScheduleCard}>
						<Text style={styles.paymentScheduleTitle}>
							Jadwal Pembayaran
						</Text>
...
const createStyles = (isDark: boolean, width: number) =>
	StyleSheet.create({
		container: {
			flex: 1,
