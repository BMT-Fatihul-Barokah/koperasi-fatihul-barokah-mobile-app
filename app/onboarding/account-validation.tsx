...
export default function AccountValidationScreen() {
	...
	const isFormValid = useMemo(() => {
		return (
			...
			rawAccountNumber.trim().length >= 6
		); // Minimum 6 digits for account number
	...
	const handleValidate = async () => {
		...
		if (
			...
			Alert.alert(
				"Error",
				"Mohon masukkan nomor rekening Anda dengan benar"
			);
			return;
		}
		...
		try {
			...
			console.log(
				`Attempting to validate account: ${fullName} with rekening: ${accountNumber}`
			);

			// Validate account against database
			const anggota = await DatabaseService.validateAccount(
				fullName,
				accountNumber
			);
			...
			if (!anggota) {
				Alert.alert(
					"Error",
					"Akun tidak ditemukan. Pastikan nama lengkap dan nomor rekening benar."
				);
				setIsValidating(false);
				return;
			}
			...
			try {
				const account =
					await DatabaseService.createOrUpdateAccount(
						anggota.id,
						phoneNumber
					);
				...
				// Check if PIN exists
				if (account.pin) {
					// PIN already exists, login and navigate to dashboard
					await storage.setItem(
						"koperasi_auth_account_id",
						account.id
					);
				...
				console.error(
					"Error creating/updating account:",
					accountError
				);
				Alert.alert(
					"Error",
					"Gagal membuat atau memperbarui akun. Silakan coba lagi."
				);
			}
		} catch (error) {
			...
			>
				<View style={styles.content}>
					...
					<Text style={styles.subtitle}>
						Mohon masukkan nama lengkap Anda sesuai
						dengan yang terdaftar di koperasi dan nomor
						rekening Anda
					</Text>
					...
					<View style={styles.infoContainer}>
						<Text style={styles.infoText}>
							Catatan: Sistem memvalidasi data yang
							diperbarui setiap hari pukul 15.00.
							Transaksi setelah waktu ini akan
							tercermin pada hari berikutnya.
						</Text>
					</View>
				</View>
