...
export const DatabaseService = {
	...
	async validateAccount(
		...
		try {
			Logger.debug("Database", "Validating account", {
				namaLengkap,
				nomorRekening,
			});
			...
			Logger.debug(
				"Database",
				"No exact match found, trying case-insensitive search"
			);
	...
	async createOrUpdateAccount(
		...
		try {
			Logger.debug("Database", "Creating/updating account", {
				anggotaId,
				nomorTelepon,
			});
			...
			const {
				data: existingAccountForAnggota,
				error: fetchAnggotaError,
			} = await supabase
				.from("akun")
				.select("*")
			...
			if (
				fetchAnggotaError &&
				fetchAnggotaError.code !== "PGRST116"
			) {
				// PGRST116 is "no rows returned" error
				Logger.error(
					"Database",
					"Error fetching account for anggota",
					fetchAnggotaError
				);
			...
			if (
				existingAccountForAnggota &&
				existingAccountForAnggota.nomor_telepon !== nomorTelepon
			) {
				Logger.warn(
					"Database",
					"Anggota already linked to another phone number",
					{
						anggotaId,
						existingPhone:
							existingAccountForAnggota.nomor_telepon,
					}
				);
				return null;
			}
			...
			const {
				data: existingAccountForPhone,
				error: fetchPhoneError,
			} = await supabase
				.from("akun")
				.select("*")
			...
			if (fetchPhoneError && fetchPhoneError.code !== "PGRST116") {
				...
				Logger.error(
					"Database",
					"Error fetching account for phone",
					fetchPhoneError
				);
				throw new Error("Failed to fetch account");
			}
			...
			if (
				existingAccountForPhone &&
				existingAccountForPhone.anggota_id !== anggotaId
			) {
				Logger.warn(
					"Database",
					"Phone number already linked to another anggota",
					{
						nomorTelepon,
						existingAnggotaId:
							existingAccountForPhone.anggota_id,
					}
				);
				return null;
			}
			...
			if (existingAccountForAnggota) {
				Logger.debug(
					"Database",
					"Found existing account, updating",
					{ accountId: existingAccountForAnggota.id }
				);
				...
				Logger.debug(
					"Database",
					"No existing account found, creating new account"
				);
				// Create new account
				const { data, error } = await supabase
					.from("akun")
			...
			Logger.error(
				"Database",
				"Error in createOrUpdateAccount",
				error
			);
			throw error;
		}
	...
	async setAccountPin(accountId: string, pin: string): Promise<boolean> {
		try {
			Logger.debug("Database", "Setting account PIN", {
				accountId,
			});
			...
			Logger.debug("Database", "PIN set successfully", {
				accountId,
			});
			return true;
		} catch (error) {
	...
	/**
	 * Update PIN for an account after verifying old PIN
	 */
	async updateAccountPin(
		accountId: string,
		oldPin: string,
		newPin: string
	): Promise<{ success: boolean; message: string }> {
		try {
			Logger.debug("Database", "Updating account PIN", {
				accountId,
			});
			...
			if (verifyError) {
				Logger.error(
					"Database",
					"Error verifying old PIN",
					verifyError
				);
				return {
					success: false,
					message: "Gagal memverifikasi PIN lama",
				};
			}

			if (account.pin !== oldPin) {
				Logger.debug("Database", "Old PIN does not match");
				return {
					success: false,
			...
			Logger.debug("Database", "PIN updated successfully", {
				accountId,
			});
			return { success: true, message: "PIN berhasil diperbarui" };
		} catch (error) {
			Logger.error("Database", "Error in updateAccountPin", error);
			return {
				success: false,
				message: "Terjadi kesalahan saat memperbarui PIN",
			};
		}
	},
	...
	async getAccountBalance(anggotaId: string): Promise<number> {
		try {
			...
			if (
				cachedData &&
				now - cachedData.timestamp < BALANCE_CACHE_EXPIRATION
			) {
				Logger.debug("Database", "Using cached balance", {
					anggotaId,
			...
			if (!viewError && viewData && viewData.length > 0) {
				const totalBalance = viewData.reduce(
					(sum, account) => sum + (account.saldo || 0),
					0
				);

				// Cache the result
				balanceCache.set(anggotaId, {
				...
				Logger.debug(
					"Database",
					"Balance calculated from view",
					{
						anggotaId,
						balance: totalBalance,
						accountCount: viewData.length,
					}
				);
				return totalBalance;
			}
			...
			if (viewError) {
				Logger.debug(
					"Database",
					"Falling back to direct query for balance",
			...
			if (!data || data.length === 0) {
				...
				Logger.debug("Database", "No active tabungan found", {
					anggotaId,
				});
			...
			Logger.debug("Database", "Balance calculated", {
				anggotaId,
				balance: totalBalance,
	...
	async getAccountDetails(
		accountId: string
	): Promise<{ account: Akun; member: Anggota; balance: number } | null> {
		try {
			Logger.debug("Database", "Fetching account details", {
				accountId,
			});

			// Try to get account and member data in a single query using join
			const { data: accountWithMember, error: joinError } =
				await supabase
					.from("akun")
					.select(
						`
        ...
        `
			...
			if (joinError) {
				Logger.error(
					"Database",
					"Error getting account with member data",
					joinError
				);
				...
				if (!account) {
					Logger.warn("Database", "Account not found", {
						accountId,
					});
					return null;
				}
				...
				if (!member) {
					Logger.warn("Database", "Member not found", {
			...
			if (!accountWithMember || !accountWithMember.anggota) {
				Logger.warn("Database", "Account or member not found", {
					accountId,
				});
				return null;
			}
